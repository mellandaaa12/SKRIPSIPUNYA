...
export default function ForumDiskusiGuru() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const filtered = (classData.classes || []).filter((c: any) => c.teacher_id === user.id);
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      // Notify students if it's a class post
      if (!isGlobal && classId && user?.id) {
        await notifyClass(classId, user.id, "post", `Guru ${user.name} membagikan kiriman baru di forum kelas.`, "/forum");
      }
    ...
    try {
      ...
      if (post && post.user_id !== user?.id && user?.id) {
        await createNotification({
          ...
          actor_id: user.id,
          type: "comment",
          content: `${user.name} mengomentari postingan Anda.`,
          link: "/forum"
        });
      }
        ...
        <div className="px-4 py-6 max-w-7xl mx-auto lg:px-8 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-3 space-y-4">
              <div className="bg-white/85 backdrop-blur-20 border border-white/95 rounded-[2.5rem] p-6 shadow-[0_8px_32px_-4px_rgba(0,119,182,0.1)] animate-slideIn">
                ...
                <div className="space-y-2">
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-[1.5rem] transition-all font-semibold text-sm ${activeTab === "global" ? "bg-gradient-to-r from-[#0077B6] to-[#0077B6] text-white shadow-lg shadow-blue-200/50" : "text-[#0077B6] hover:bg-white/60"}`}
                  >
                  ...
                  <div className="pt-4 pb-2 px-2 border-t border-gray-100">
                    <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Kelas Saya</span>
                  </div>
                  ...
                  {teacherClasses.map(c => (
                    <button 
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-[1.5rem] transition-all font-semibold text-sm ${activeTab === c.id ? "bg-gradient-to-r from-[#0077B6] to-[#0077B6] text-white shadow-lg shadow-blue-200/50" : "text-[#0077B6] hover:bg-white/60"}`}
                    >
                      <Hash className="w-5 h-5 flex-shrink-0" />
                      <div className="text-left overflow-hidden">
                        <p className="truncate leading-tight">{formatClassDisplayName(c)}</p>
              ...
              {/* Action Card */}
              <div className="bg-gradient-to-br from-[#0077B6] to-[#0077B6] rounded-[2.5rem] p-8 shadow-lg shadow-blue-200/50 text-white animate-fadeIn">
                <h4 className="font-bold text-lg mb-2">Buat Diskusi</h4>
                <p className="text-xs opacity-80 mb-6 leading-relaxed">Berikan arahan atau tanyakan progres kepada siswa Anda secara langsung.</p>
                <button 
                  onClick={() => setIsAddPostModalOpen(true)}
                  className="w-full py-4 bg-white text-[#0077B6] rounded-[1.5rem] font-bold text-sm shadow-sm hover:bg-gray-50 transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="lg:col-span-6 space-y-6">
              {loading ? (
                ...
                <div className="bg-white/85 backdrop-blur-20 border border-white/95 rounded-[2.5rem] p-20 flex flex-col items-center justify-center text-center shadow-[0_8px_32px_-4px_rgba(86,182,198,0.1)]">
                  ...
                  <p className="text-sm text-gray-500 max-w-xs mt-2">Jadilah yang pertama untuk membagikan informasi di sini.</p>
                </div>
              ) : (
                posts.map((post, idx) => (
                  <motion.div
                  ...
                  >
                    <div className="flex gap-4">
                      <div className="relative flex-shrink-0">
                        <div className={`h-12 w-12 rounded-full bg-gradient-to-br ${post.userRole === 'admin' ? 'from-red-400 to-red-600' : 'from-[#0077B6] to-[#0077B6]'} flex items-center justify-center text-white text-lg font-bold shadow-sm`}>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`text-[9px] font-bold text-white px-2 py-0.5 rounded-full ${post.userRole === 'admin' ? 'bg-red-500' : 'bg-[#0077B6]'}`}>
                        ...
                        <div className="space-y-3 pt-4 border-t border-gray-100/50">
                          ...
                          <div className="flex gap-3 mt-4">
                            <input 
                              ...
                              className="flex-1 px-5 py-2.5 rounded-full bg-gray-50/80 border border-gray-200 text-black text-xs focus:ring-2 focus:ring-[#0077B6]/20 outline-none transition-all"
                              onKeyDown={(e) => e.key === "Enter" && handleComment(post.id)}
                            />
                            <button 
                              onClick={() => handleComment(post.id)}
                              disabled={!commentText[post.id]?.trim()}
                              className="p-2.5 bg-[#0077B6] text-white rounded-full hover:bg-[#0077B6] transition-all shadow-md disabled:opacity-50"
                            >
              ...
              {activeTab !== "global" && (
                <div className="bg-white/85 backdrop-blur-20 border border-white/95 rounded-[2.5rem] p-8 shadow-[0_8px_32px_-4px_rgba(0,119,182,0.1)] animate-fadeIn">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Anggota</h3>
                    <span className="text-[10px] font-bold bg-blue-50 text-[#0077B6] px-3 py-1 rounded-full">
                      {classMembers.length}
                    </span>
                  </div>
                    ...
                    {loadingMembers ? (
                      ...
                      <div key={member.id} className="flex items-center gap-3 group">
                        ...
                        <div className="flex-1 overflow-hidden">
                          <p className="text-xs font-bold text-[#0077B6] truncate group-hover:text-[#0077B6] transition-colors">{member.name}</p>
                          <p className="text-[9px] text-gray-400 font-medium">
                            {isOnline(member.updated_at) ? 'Sedang Aktif' : 'Offline'}
                          </p>
                        </div>
                      </div>
                    ))}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0077B6]/40 backdrop-blur-md p-4">
            ...
            >
              <div className="p-8 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-[#0077B6]">Buat Diskusi Baru</h3>
                  <p className="text-xs text-gray-500 font-medium mt-1">
                    Postingan ini akan terlihat di {activeTab === "global" ? "seluruh pengguna" : "anggota kelas"}
                  </p>
                </div>
              ...
              <div className="p-8 space-y-6">
                <div className="flex items-center gap-4 p-5 bg-blue-50/50 rounded-3xl border border-blue-100/50">
                  ...
                  <div>
                    <p className="text-base font-bold text-[#0077B6]">
                      {activeTab === "global" ? "Forum Global" : `${teacherClasses.find(c => c.id === activeTab)?.name} - ${teacherClasses.find(c => c.id === activeTab)?.subject}`}
                    </p>
                  </div>
                </div>
                ...
                <textarea 
                  ...
                  placeholder="Apa yang ingin Anda bagikan kepada siswa hari ini?"
              ...
              <div className="p-8 pt-0">
                <button 
                  ...
                  className="w-full py-5 bg-gradient-to-r from-[#0077B6] to-[#0077B6] text-white rounded-[1.5rem] font-bold shadow-lg shadow-blue-200/50 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
